...
function message_list_to_conversation_list(messages) {
    return messages.map((message) => {
        const out = {role: (!message.user_id) ? "system" : "user", content: message.content};
        return out;
    });
}
...
function message_list_to_conversation_string(messages) {
    ...
    messages.forEach((message) => {
        conversation += `${(!message.user_id) ? "ALICIA" : "USER"}: ${message.content}`;
    });
...
async function get_next_message_from_Alicia(messages, prev_summary) {
    ...
    const system_prompt = Alicia_base_system_prompt + prev_summary;
